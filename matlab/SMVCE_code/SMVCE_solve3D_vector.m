...
% maskdata=ones(size(data));
maskdata=zeros(size(data));
for i=1:size(data,3)
   maskdatai=filter2(ones(windowsize*6),abs(data(:,:,i)))~=0;
   maskdata(:,:,i)=maskdatai;
end
...
para_sm=zeros(row,col,unknum-3);
var_e=zeros(row,col);
var_n=zeros(row,col);
var_u=zeros(row,col);
...
%     for j=1424:col
        ...
        [L_i0,iis,jjs,SHPcounti]=getL(data,i,j,windowsize,coor,fault,flag_smad,flag_adpws,maskdata);
        SHPcount(i,j,:)=SHPcounti;
        %得到用于方差分分量估计的观测值，设计矩阵和权重矩阵
        
        [L,B,k,P]=getLBkP(L_i0,iis,jjs,Bgeo,xx,yy,x_m,y_m,fsmpara,dem,i,j,flag_interWeight,flag_BOI,diswa,diswb);
        if sum(k>=0.1*windowsize^2)<2 %||rank(B0'*B0)<unknum
            continue;
        end
        
        %方差分量估计
        %VCE DETERMINE THE WEIGHT
        [sita,P_vce,unkn]=SMVCE_vce_vector(L,B,P,enangle(i,j));
...
%% other functions
function [L,B,k,P]=getLBkP(L_i0,iis,jjs,Bgeo,xx,yy,x_m,y_m,fsmpara,dem,i,j,flag_interWeight,flag_BOI,diswa,diswb)
...
function [L_i0,iis,jjs,SHPcounti]=getL(data,i,j,windowsize,coor,fault,flag_smad,flag_adpws,maskdata)
[row,col,data_num]=size(data);
...
mask_all_ij=squeeze(maskdata(i,j,:));
