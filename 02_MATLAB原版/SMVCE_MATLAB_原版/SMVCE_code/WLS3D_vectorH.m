function Result_wls=WLS3D(pDATA,enangle)
%WLS_3D
...
parfor i=1:row
    ...
    for j=1:col
        ...
        vi=Bgeo_i*x-L_i;
        d0=vi'*P_i*vi/(length(L_i)-length(x));
        vari=zeros(3,1);
        varii=sqrt(diag(d0*NN));
        vari(dims)=varii(1:ndim);
        var_enu(i,j,:)=vari;
    end
    if flag==0
