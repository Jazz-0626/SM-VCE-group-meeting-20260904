function Result_smvce=SMVCE_solve3D(pDATA,enangle)
%SMVCE_solve3D
...
DATA=load(pDATA,'data','inc','azi','losazienu',...
    ...
    'windowsize','flag_BOI','fault','fsmpara');
% load('enangle1s.mat','enangle1');
% enangle=enangle1;
flagen=1;
if ~exist('enangle','var')
    enangle=zeros(coor.nlines,coor.width);
    flagen=0;
end
dims=[1,3];
...
unknum=ndim*(1+fsmpara);


defo_enu=zeros(row,col,3);
para_sm=zeros(row,col,unknum-ndim);
var_enu=zeros(row,col,3);
...
Bgeo=lk_vecH(azi,inc,losazienu,leftorright,enangle);
...
if flag==1
    ...
    progressBar.message = 'Solving 3-D disp. by SM-VCE: ';
...
parfor i=1:row
    ...
    for j=1:col
        ...
        [L,B,k,P]=getLBkP(L_i0,iis,jjs,Bgeo,xx,yy,x_m,y_m,fsmpara,dem,i,j,flag_interWeight,flag_BOI,diswa,diswb,dims);
        if sum(k>=0.1*windowsize^2)<ndim %||rank(B0'*B0)<unknum
            continue;
        end
        ...
        if sum(sita)==0
            ...
            fprintf(fid,'row:%d,col:%d are not calculated for the  divergence of VCE',i,j);
            fclose(fid);
        end
        ...
        Bgeo_var=Bgeo_var(:,dims);
        Bgeo_var(isnan(Bgeo_var))=0;
        sita(sita==0)=nan;
        d0=nanmean(sita);
        var_enui=d0*pinv(Bgeo_var'*diag(p)*Bgeo_var);
        var_enuii=zeros(3,1);
        var_enuii(dims)=sqrt(diag(var_enui));        
        var_enu(i,j,:)=var_enuii;
    end
    if flag==0
...
if flagen==1
    defo_n=defo_enu(:,:,1).*sin(enangle);
else
    defo_n=defo_enu(:,:,2);
end
defo_e=defo_enu(:,:,1).*cos(enangle);
defo_u=defo_enu(:,:,3);
...
%% other functions
function [L,B,k,P]=getLBkP(L_i0,iis,jjs,Bgeo,xx,yy,x_m,y_m,fsmpara,dem,i,j,flag_interWeight,flag_BOI,diswa,diswb,dims)
...
for kk=1:data_num
    ...
    if fsmpara==3
        ...
        B_i=get_design_mat(Bgeo_i,dx_i,dy_i,dz_i,dims);
    else
        B_i=get_design_mat(Bgeo_i,dx_i,dy_i,dims);
    end
    tem=L_i0{kk};
    tem_B=B_i;
