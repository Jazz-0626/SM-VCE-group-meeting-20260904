function B=get_design_mat_2D(Bgeo,de,dn,du)
%get_design_mat
...
if nargin==4
    ...
    for i=1:data_num
        ...
        cn=ci(:).*dn(:);
        cu=ci(:).*du(:);
        B(1+m*n*(i-1):m*n*i,:)=[a,c,ae,an,au,ce,cn,cu];
    end
else
    %considering 2-D SM
    B=zeros(m*n*data_num,6);
    for i=1:data_num
        ai=Bgeo(:,:,1+3*(i-1));
        bi=Bgeo(:,:,2+3*(i-1));
        ci=Bgeo(:,:,3+3*(i-1));
        ...
        ce=ci(:).*de(:);
        cn=ci(:).*dn(:);
        B(1+m*n*(i-1):m*n*i,:)=[a,c,ae,an,ce,cn];
    end
end
end
